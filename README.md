...
チャンネル (channel) は
指定された要素型を[送信](#送信文)と[受信](#受信演算子)する値によって通信する
...
- 構造体タグを無視し（下記参照），`x` の型と `T` は
[定義型](#型定義)ではないポインター型であり，
...
([ラベル付き](#ラベル式)) [`fallthrough` 文](#fallthrough文)
の場合がある．
...
更新文 (post statement; `PostStmt` @@@反復文・後処理文...) が指定される．
...
関数値とパラメータは，呼び出し側のゴルーチンの内で，[通常通り評価される](#呼び出し)が，
通常の呼び出しとは異なり，
...
`goto` 文を実行するとき，
`goto` 時点ではまだ
[スコープ](#宣言とスコープ)内になかった変数がスコープ内に入ってはならない．
例えば，
```go
	goto L  // BAD
	v := 3
...
親関数が
リテラルのスコープに属する
[名前付き復帰パラメータ](#関数型)を持つとき，
遅延関数は，それらが復帰する前に関数パラメータはアクセスされ，変更される．
遅延関数が復帰値をもつとき，
関数が完了すると破棄される．
...
ビルトイン関数（組み込み関数; built-in function) は
[事前宣言](#事前宣言された識別子)されている．
ビルトイン関数は，
他の関数のように呼び出されるが，
...
それぞれ[パラメータ受け渡しのルール](#passing_arg_to_params)が適用される．
特別な場合として，
`append` はまた，
最初の引数が，`[]byte` 型に代入可能で，
