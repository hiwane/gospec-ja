...
本文書は，[The Go Programming Language Specification](https://golang.org/ref/spec) version 2021/02/10 のなんちゃって日本語訳である．
...
小文字の `Production` 名 (`production_name`) は，字句トークンを識別するために使用する．
...
以下の文字列は[演算子](#演算子) ([代入演算子](#代入) (assignment operators) を含む) と区切り (punctuation) である．
...
インターフェース `T` は
メソッド仕様の代わりに
(修飾された可能性のある) インターフェース型名 `E` を使用できる．
これは，`T` における**埋め込み** (embedding) インターフェース `E` と呼ばれる．
`T` の[メソッド集合](#メソッド集合)は
`T` の明示的に宣言されたメソッド集合と，
`T` の埋め込みインターフェースのメソッド集合の**和集合**である．
...
チャンネルは[代入](#代入)か明示的な[変換](#変換)によって送信のみか受信のみに制限される場合がある．
...
変数は以下の[代入](#代入)規則に従った式によって初期化される．
...
[代入](#代入)や初期化で使用される型 `map[K]V` のマップ `a` のインデックス式
...
パッケージ [unsafe](#package_unsafe)は制限された状況下で，この機能を実装する．
