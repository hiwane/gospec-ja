...
インターフェース型 `T` は `T` を直接的，または，間接的に含むか埋め込む型要素を埋め込んではならない．
...
定義型は，それに関連付けられた[メソッド](#メソッド宣言)をもつことができる．
...
事前宣言された値 `nil` は，明示的な型を持たない変数の初期値として使用できない．
...
[`if`文](#if文)，
[`for`文](#for文)，
[`switch`文](#switch文)の初期化のようにローカル一時変数の宣言に使用できる．
...
メソッド宣言 (method declaration) は**メソッド名** (method name) である識別子に，
メソッドをバインド (bind) し，
...
各要素の前には，オプションで対応するキー `Key` を置くことがある．
...
`LiteralType` の[コア型](#コア型) `T` は，
構造体型，配列型，スライス型，マップ型でなければならない
(構文は，その型が `TypeName` として与えられる場合を除いて，
この制約を強制する．)．
要素とキーの型は，
型 `T` のそれぞれのフィールド型，要素型，キー型に[代入可能](#代入可能性)でなければならない．
さらなる変換はない．
キーは，
構造体のフィールド名，
...
- キーを全く含まない要素リスト `ElementList` は，
フィールドが宣言された順序で各構造体フィールドに対する要素がリストされなければならない．
- ある要素がキーをもつならば，すべての要素がキーをもたなければならない．
...
- キーをもつ要素は，そのキーをインデックスとして使用する．
そのキーは `int` 型の値で[表される](#表現可能性)非負定数でなければならず，
型付けされている場合には， [整数型](#数値型)でなければならない．
...
エラーになる．
表記 `...` は要素のインデックスの最大値に 1 を加えた配列の長さの配列を指定する．
...
スライスリテラルは，
基底配列リテラル全体を記述する．
したがって，スライスリテラルの長さと容量は要素のインデックスの最大値に 1 を加えたものとなる．
スライスリテラルは，次の形式をもつ．
...
キーワードと `if`文, `for`文, `switch`文のブロックの開き波括弧 `{` の間にオペランドとして現れ，かつ，
...
`T` の埋め込みフィールド `A` において宣言されたフィールド `f` またはメソッド `f` の深さは，
...
そのような関数は，レシーバーを介して間接的に，
基底メソッドにレシーバーとして渡す値を生成する．
...
`a` がマップでも型パラメーターでもない場合
- インデックス `x` は型なし定数か
その[コア型](#コア型)は，[整数](#数値型)でなければならない．
...
`a` が[型パラメーター型](#型パラメーター宣言) `P` の場合
- インデックス式 `a[x]` は `P` の型集合に含まれるすべての型の値に対して有効でなければならない．
- `P` の型集合に含まれるすべての型の要素型は，同一でなければならない．
この文脈では，文字列型の要素型は `byte` である．
- `P` の型集合にマップ型がある場合，その型集合に含まれるすべての型はマップ型でなければならず，
それぞれのキー型はすべて同一でなければならない．
- `a[x]` は配列，スライス，または文字列のインデックス `x` の要素であるか，
型引数のキー `x` をもつマップ要素である．
`P` はインスタンス化された型引数であり，
`a[x]` の型は，（同一の）要素型の型である．
- `P` 型集合が文字列型を含む場合，
`a[x]` には代入してはならない．

上記以外の場合，`a[x]` は不当である．

[代入文](#代入文)や初期化で使用される型 `map[K]V` のマップ `a` のインデックス式
...
文字列，配列，配列へのポインター，または，スライスから部分文字列，または，スライスを構築する．
...
インデックスは `0 <= low <= high <= max <= cap(a)` のとき，
...
[ランタイムパニック](#ランタイムパニック)が発生する．
言い換えると，
`x` の動的な型を実行時にしかわからないとしても，
正しいプログラムでは，
`x.(T)` の型は `T` であることがわかっている．
...
そうでなければ，
その渡された値は,
連続する要素が実際の引数 (すべて `T` に[代入可能](#代入可能性)でなければならない) である新しい基底配列にもつ，
型 `[]T` の新しいスライスである．
そのため，
...
シフトカウントの上限はない．
シフトは，左のオペランドがシフトカウント `n` に対して，
`n` 回 1 シフトされるように動作する．
結果として，`x << 1` は `x * 2` と同じであり，
`x >> 1` は負の無限大に向かって切り捨てられるが，
`x / 2` と同じである．
...
動的な値が等しいか，
どちらも `nil` の場合に等しい．
- 非インターフェース型 `X` の値 `x` とインターフェース型 `T` の値 `t` は，
...
オペランドは**アドレス可能** (addressable)でなければならない．
つまり，
...
特別なケースとして，
整数定数 `x` は，
非定数 `x` のように同じルールを使用して明示的に[文字列型](#文字列型)に変換できる．
...
パッケージ [unsafe](#パッケージ `unsafe`)は制限された状況下で，この機能を実装する．
...
連続した要素がその文字列のバイトたちであるスライスが生成される．
...
同じ種類の型なし定数を復帰する．
つまり，
ブール定数，整数定数，浮動小数点定数，複素数定数，文字列定数である．
...
定数式はいつも正確に評価される；
中間値と定数自身は，
言語が事前宣言した型によってサポートされる精度より遥かに大きな精度を要求する場合もある．
以下は，正当な宣言である．
...
`TypeSwitchGuard` は簡潔な変数宣言 `SimpleStmt` を含めることができる．
その形式が使用されると，
その変数は
各句の[暗黙的なブロック](#ブロック)の
...
型の代わりに，
事前宣言された識別子 [`nil`](#事前宣言された識別子) が使用されることがある；
...
`defer` 文を囲む親関数 (surrounding function) が
...
`b` と `a` の間に初期化されるか，
`a` の後に初期化されるかどうか
したがって，`sideEffect()` が呼び出されるタイミング (moment) も指定されていない．
...
関数 `Offsetof` は構造体のアドレスを基準とした相対バイトで，フィールドオフセットを復帰する．
